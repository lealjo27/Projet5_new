...
# # Chargement des datas -- avec le csv
# try:
...
def predict(id_employe: PositiveInt, db : Session = Depends(get_db)):
    ...
    donnees_pour_ia = {}
    for nom_csv, nom_bdd in MAPPING_COLONNES.items():
            # On dit à Python : prend la valeur de la colonne 'nom_bdd' 
            # et range-la sous le nom 'nom_csv'
            donnees_pour_ia[nom_csv] = getattr(employe, nom_bdd)
    
    features = pd.DataFrame([donnees_pour_ia])
